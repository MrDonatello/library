...
class BookAuthorAdd extends Component {
    ...
    async handleSubmit(event) {
        ...
        await fetch('/library/bookAuthor/add/' + item.id, {
            method: 'PUT',
            headers: {
    ...
    render() {
        const {item} = this.state;
        const title = <h2>{'Add Author Book'}</h2>;
...
export default withRouter(BookAuthorAdd);